...
// Principals are named by SID throughout. A name has to be resolved first, and which
// spellings resolve is not what you would guess: measured on a Turkish install, icacls
// takes the English "Administrators" and "Everyone" and refuses the localised
// "Yöneticiler" and "Herkes", both with error 1332 and nothing granted. A SID needs no
// lookup at all, so it is right on every machine without anyone having to know which of
// those two rules that machine follows.
...
Run run(const QString &program, const QStringList &arguments)
{
    ...
    result.output = Console::decode(process.readAll()).trimmed();
    return result;
}
